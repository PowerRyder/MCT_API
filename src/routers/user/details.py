...
def dashboard_details(token_payload: any = Depends(get_current_user)):
    try:
        ...
        if len(dataset)>0 and len(dataset['rs']):
            ds = dataset['rs']
            if(ds.iloc[0].loc["valid"]):
                return {'success': True, 'message': OK, 'data': dataFrameToJsonObject(ds), 'income': dataFrameToJsonObject(dataset['rs1']), 'wallet_balances': dataFrameToJsonObject(dataset['rs2']) }
