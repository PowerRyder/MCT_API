...
from src.data_access.income import monthly_bonus as data_access
...
@router.get('/get_monthly_bonus_payouts', dependencies=[Depends(RightsChecker([227, 228]))])
def get_monthly_bonus_payouts():
    try:
        dataset = data_access.get_monthly_bonus_payouts()
        # print(dataset)
        if len(dataset) > 0:
            ds = dataset['rs']
...
@router.post('/get_monthly_bonus', dependencies=[Depends(RightsChecker([227, 228]))])
def get_monthly_bonus(req: GetMonthlyBonus_Request, token_payload: any = Depends(get_current_user)):
    try:
        ...
        dataset = data_access.get_monthly_bonus(req=req, match_exact_user_id=match_exact_user_id)
        # print(dataset)
        if len(dataset) > 0:
            ds = dataset['rs']
            return {'success': True, 'message': OK, 'data': dataFrameToJsonObject(ds),
