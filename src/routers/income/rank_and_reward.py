...
from src.data_access.income import rank_and_reward as data_access
...
@router.get('/get_ranks', dependencies=[Depends(RightsChecker([217, 219]))])
def get_ranks():
    try:
...
@router.post('/get_rank_and_reward_income', dependencies=[Depends(RightsChecker([217, 219]))])
def get_rank_and_reward_income(req: GetRankAndRewardIncome_Request, token_payload: any = Depends(get_current_user)):
    try:
        ...
        dataset = data_access.get_rank_and_reward_income(req=req, match_exact_user_id=match_exact_user_id)
        # print(dataset)
        if len(dataset) > 0:
            ds = dataset['rs']
            return {'success': True, 'message': OK, 'data': dataFrameToJsonObject(ds),
