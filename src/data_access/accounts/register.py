...
def isSponsorValid(sponsor_id:str):
    return execute_query("call usp_is_sponsor_valid(_sponsor_id => %s)", (sponsor_id, ))
    
def isUplineValid(upline_user_id:str):
    return execute_query("call usp_is_upline_valid(_upline_user_id => %s)", (upline_user_id, ))
    
def doesUserIdExist(user_id:str):
    return execute_query("call usp_does_user_id_exists(_user_id => %s)", (user_id, ))

def updateJoiningMailAndSmsStatus(user_id, is_email_sent, is_sms_sent):
    return execute_query("call usp_joining_mail_and_sms_status(_user_id => %s, _is_email_sent => %s, _is_sms_sent => %s)", (user_id, is_email_sent, is_sms_sent))

def register(data: Register, crypto_deposit_addresses):
    # print(data)
    return execute_query("""
    call usp_register(_sponsor_user_id => %s, _password => %s, _user_id => %s, _pin_number => %s, _pin_password => %s, _upline_user_id => %s,  _side => %s, _name => %s, _dob => %s, 
        _marital_status => %s, _gender => %s, _address => %s, _district => %s, _state => %s, _country => %s, _pincode => %s, _mobile => %s, _email => %s, _nominee_title => %s, _nominee_name => %s,
        _nominee_relationship => %s, _ifscode => %s, _bank_name => %s, _branch_name => %s, _bank_account_number => %s, _account_holder_name => %s, _pan_card_number => %s, _crypto_deposit_addresses => %s::json)""", 
    (
